...
- `parseSwapSpec(value)` - Parse the canonical serialized `hx-swap` value
- `runViewTransition(mode, root, update, sourceElement)` - Run a document- or target-scoped transition with standard fallback and lifecycle events
